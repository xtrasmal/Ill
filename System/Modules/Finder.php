<?php namespace Ill\System\Modules;
...
class Finder {
	...
	{
		if ($this->app['config']->get('ill.mode') == 'auto')
		{
			$this->app['modules']->scan();
		}
		elseif ($this->app['config']->get('ill.mode') == 'manifest')
		{
	...
	public function scan()
	{
		...
		$modulesPaths = $this->app['config']->get('ill.path');
	...
	{
		if ( ! $config) $modules = $this->app['config']->get('ill.modules');
		else            $modules = $config;
		...
		if ($modules)
		{
			foreach ($modules as $key => $module)
			{
				...
				// The path
				$path = base_path($this->app['config']->get('ill.path') . '/' . $name);
	...
	{
		if ($this->app['config']->get('ill.debug'))
		{
			$namespace = 'MODULES';
